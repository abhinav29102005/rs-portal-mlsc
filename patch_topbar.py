...
export function Topbar({ user, notificationCount = 0 }: TopbarProps) {
  ...
  return (
    <header className="fixed top-0 left-[260px] right-0 h-16 z-30 flex items-center justify-between px-6 bg-noir-950/80 backdrop-blur-xl border-b border-white/[0.04]">
      ...
      <ProductTour startTour={startTour} onTourEnd={() => setStartTour(false)} />
      ...
      <div className="flex items-center gap-3">
        ...
        <button 
          ...
          className="btn btn-ghost text-red-400 hover:bg-red-500/10 flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm"
          id="demo-tour-button"
        >
          <PlayCircle size={16} />
